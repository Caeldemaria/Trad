...
class UserController extends Controller
{
    ...
    public function dashboard()
    {
        ...
        //  refferMoney($user->id, $user->refferedBy, 'interest', 500);
    ...
    {
        $pageTitle = 'Reword';
        return view($this->template . 'user.reword', compact('pageTitle'));
    }

     public function clm($id)
    {
        $reward = Reward::find($id);
        $model = new RewardRequest();
        $teamSize = depoTeamSize(auth()->id(), 1);
        
        $totalTeam = teamSize(auth()->id(), 1)+teamSize(auth()->id(), 2)+teamSize(auth()->id(), 3);

        //Already try
        $alreadyTry = RewardRequest::where('user_id', \auth()->id())->where('reward_id', $id)->first();
        if ($alreadyTry){
            return back()->with('error', 'You already get this reward');
        }
        
        if ($totalTeam > $reward->tteam){
            if ($teamSize >= $reward->size){
                $model->user_id = \auth()->id();
                $model->reward_id = $id;
                $model->amount = $reward->bonus;
                $model->created_at = now();
                $model->updated_at = now();
                $model->save();
    
                auth()->user()->balance = auth()->user()->balance + $reward->bonus;
                auth()->user()->save();
    
                return back()->with('success', 'You have received the '.$reward->name . ' bonus');
            }else{
                return back()->with('error', 'Opps! Please improve your active member.');
            }
        }else
        {
            return back()->with('error', 'Opps! Please improve your team.');
        }
    }
...
return back()->with('success', 'Successfully Updated Profile!');
    ...
    {
        $user = User::where('ref_id', $request->reffer)->first();
        if ($user) {
    ...
    {
        if ($request->tamount == null || $request->tamount == ''){
        ...
        if ($request->tamount < $general->trading_min_amount){
            $msg = "Enter minimum 30$ for starting BOT";
            $notify[] = ['error', $msg];
            return back()->withNotify($notify);
        }
        ...
        $miningAmount = 0;
        if ($user->balance < $request->tamount) {
        ...
        $miningAmount = $request->tamount; // user balance
        $user->balance = $user->balance - $miningAmount;
        $user->trading_balance = $user->trading_balance + $miningAmount;
...
//        }// set date as tomorrow
        $user->collect_date = Carbon::now()->addMinutes($general->minute);
        $user->save();
