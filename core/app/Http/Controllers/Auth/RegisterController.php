...
class RegisterController extends Controller
{
    ...
    public function register(Request $request)
    {
        ...
        if ($request->reffered_by) {
            $userRefer = User::where('ref_id', $request->ref_by)->first();
            if ($userRefer){
                $referUser = User::where('username', $userRefer->username)->first();
    ...
    public function create($request, $signupBonus, $referid)
    {
        ...
        return User::create([
            ...
            'reffered_by' => $userRefer ? $userRefer->id : '',
            'ref_by' => $userRefer ? $userRefer->ref_id :'',
            'ref_id'=> rand(0,9999).rand(0,9999)
        ]);
    }
